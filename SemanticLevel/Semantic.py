...
class Semantic:
    ...
    def run(self, func_name, input_token):
        ...
        try:
            ...
            print("SemanticLevel Error! Function '" + func_name + "' not found")
            pass
    ...
    def error(self, err_type, line_number, id, expected, illegal, arg):
        ...
        if err_type == ErrorType.scoping:
            err = (
                "#" + line_number + ": SemanticLevel Error! '" + id + "' is not defined"
            )
        elif err_type == ErrorType.void_type:
            err = (
                "#"
                + line_number
                + ": SemanticLevel Error! Illegal type of void for '"
                + id
                + "'"
            )
            ...
            err = (
                "#"
                + line_number
                + ": SemanticLevel Error! Type mismatch in operands, Got '"
                + illegal
                + "' instead of '"
                + expected
                + "'"
            ...
            err = (
                "#"
                + line_number
                + ": SemanticLevel Error!Mismatch in type of argument "
                + arg
                + " for '"
                + id
                + "'. Expected '"
