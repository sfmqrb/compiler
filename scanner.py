...
class scanner:
    ...
    def get_next_token(
        ...
        while self.this_char_idx < len(self.s):
            ...
            if (
                self.next_state == STATE_SCANNER.ENDBACK
                or self.next_state == STATE_SCANNER.END
                or self.next_state == STATE_SCANNER.ERROR
            ) and self.log:
                self.raw_token_type = self.log

            to_return = self.empty_return
            if (
                self.this_state == STATE_SCANNER.END
                or self.this_state == STATE_SCANNER.ERROR
            ):
