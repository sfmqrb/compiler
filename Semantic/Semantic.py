...
class Semantic:
    ...
    def run(self, func_name, input_token):
        ...
        try:
            getattr(SemanticRoutines, "func_" + func_name)(self.parse_table.get_adr, self.temp_manager.get_temp,
                                                           input_token)
        except:
            pass
    ...
    def error(self, err_type, line_number, id, expected, illegal, arg):
        ...
        if err_type == ErrorType.scoping:
            err = "#" + line_number + ": Semantic Error! '" + id + "' is not defined"
        elif err_type == ErrorType.void_type:
            err = "#" + line_number + ": Semantic Error! Illegal type of void for '" + id + "'"
            ...
            err = "#" + line_number + ": Semantic Error! No 'while' or 'switch' found for 'break'"
        elif err_type == ErrorType.type_mismatch:
            err = "#" + line_number + ": Semantic Error! Type mismatch in operands, Got '" + illegal + "' instead of '" + expected + "'"
        elif err_type == ErrorType.type_matching:
            err = "#" + line_number + ": Semantic Error!Mismatch in type of argument " + arg + " for '" + id + "'. Expected '" + expected + "' but got '" + illegal + "' instead "
